...
'Response.Write objMD5.HEXMD5 ' This will display an MD5 hash
	...
	Set chkUser = Server.CreateObject("ADODB.Recordset")
	chkUser.ActiveConnection = MM_INVFlood_STRING 
	chkUser.Source = "SELECT * FROM [USER] where invuser ='" & request.form("username") & "' and password='" & objMD5.HEXMD5 & "'"
	chkUser.CursorType = 3
	chkUser.CursorLocation = 3
	chkUser.LockType = 1
	chkUser.Open()
	
	
	if not chkUser.eof then
		session("NameOfUser") = chkUser("name")
		session("UserID") = chkUser("UserID")
		if request.form("FromPage") = "2ndFloor" then
			response.redirect("SMPO_2nd_floor.asp")
		else
		...
		session("UserID") = ""
		if request.form("FromPage") = "2ndFloor" then
			response.redirect("index.html?flag=1&FromPage=2ndFloor")
		else
